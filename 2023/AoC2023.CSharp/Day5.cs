using AoC2023.CSharp.Extensions;

namespace AoC2023.CSharp;
...
public class Day5
{
    ...
    public void Part1()
    {
        var seedInformation = testInput.Parse();
        var map = new List<List<(long dest, long source)>>
        {
            seedInformation[0]
                .Split(':')[1]
        ...
        foreach (var cur in seedInformation.Skip(1))
        {
            if (char.IsLetter(cur[0]))
            {
                if (map.Count > 1)
                {
                    map.Last().AddRange(
                        map[^2].Where(v => !map[^1].Any(nv => nv.source == v.dest))
                            .Select(v => (v.dest, v.dest)));
                }

                map.Add(new List<(long dest, long source)>());
                continue;
            }

            var value = cur.Split(' ').Select(long.Parse).ToArray();
            var destBegin = value[0];
            var sourceBegin = value[1];
            var sourceEnd = value[1] + value[2];
            var currentRowMap = map[^2].Where(v => v.dest >= sourceBegin && v.dest < sourceEnd)
                .Select(v => (destBegin + (v.dest - sourceBegin), v.dest))
                .ToArray();
            map.Last().AddRange(currentRowMap);
        }

        map.Last().AddRange(map[^2].Where(v => !map[^1].Any(nv => nv.source == v.dest))
            .Select(v => (v.dest, v.dest)));
        var result = map.Last().MinBy(m => m.dest).dest;
        Assert.Equal(35, result);
    }

    private readonly record struct Range(long Begin, long End)
    {
        public bool Overlaps(Range other) =>
            (Begin >= other.Begin && Begin <= other.End)
            || (End >= other.Begin && End <= other.End);
        public Range GetOverlap(Range other) => new(Math.Max(other.Begin, Begin), Math.Min(other.End, End));
        
        public IEnumerable<Range> RangesNotInRange(Range other)
        {
            if (Begin < other.Begin)
            {
                yield return this with { End = Math.Min(other.Begin - 1, End) };
            }
            if (End > other.End)
            {
                yield return this with { Begin = Math.Max(other.End + 1, Begin) };
            }
        }
        
        public IEnumerable<Range> RangesNotInRanges(IEnumerable<Range> ranges) => 
            ranges.Aggregate(new List<Range>{this}, (current, other) => 
                current.SelectMany(r => r.RangesNotInRange(other)).ToList());

        public Range CalculateDestinationRange(long destBegin, long sourceBegin) => 
            new(destBegin + Begin - sourceBegin, End + destBegin - sourceBegin);
    }

    [Fact]
    public void ShouldHandleRangeOverlapMapping()
    {
        var range = new Range(79, 92);
        var map = new Range(50, 97);
        var overlap = range.GetOverlap(map);
        Assert.Equal(new Range(79, 92), overlap);
    }

    [Fact]
    public void ShouldHandleRangesNotInRange()
    {
        var range = new Range(79, 92);
        var map = new[]
        {
        ...
        var rangesNotInRanges = range.RangesNotInRanges(map);
        Assert.Empty(rangesNotInRanges);
    }

    [Fact]
    public void ShouldHandleRangesNotInRanges()
    {
        var range = new Range(42, 92);
        var map = new Range(50, 79);
        var rangesNotInRange = range.RangesNotInRange(map);
        Assert.Collection(rangesNotInRange, 
            r => Assert.Equal(new Range(42, 49), r),
            r => Assert.Equal(new Range(80, 92), r));
    }

    [Fact]
    public void ShouldCalculateDestinationRange()
    {
        var range = new Range(79, 92);
        var destination = range.CalculateDestinationRange(52, 50);
        Assert.Equal(new Range(81, 94), destination);
    }
    ...
    {
        var seedInformation = testInput.Parse();
        var map = new List<(Range dest, Range source)[]>
        {
            seedInformation[0]
                .Split(':')[1]
                .Split(' ', StringSplitOptions.RemoveEmptyEntries)
                .Chunk(2)
                .Select(c => new Range(long.Parse(c[0]), long.Parse(c[0]) + long.Parse(c[1]) - 1))
                .Select(r => (r, r))
                .ToArray()
        };

        var info = seedInformation.Skip(1).ToArray();
        for (var i = 1; i < info.Length; i++)
        {
            var rows = info.Skip(i)
                .TakeWhile(r => !char.IsLetter(r[0]))
                .ToArray();
            var previousList = map.Last();

            var overlap = rows.SelectMany(r =>
            {
                var value = r.Split(' ').Select(long.Parse).ToArray();
                var destRangeBegin = value[0];
                var sourceRange = new Range(value[1], value[1] + value[2] - 1);
                return previousList.Where(item => item.dest.Overlaps(sourceRange))
                    .Select(overlappingRange => overlappingRange.dest.GetOverlap(sourceRange))
                    .Select(overlap => (dest: overlap.CalculateDestinationRange(destRangeBegin, sourceRange.Begin), source: overlap));
            }).ToArray();
            var oneToOne = previousList.Select(range => range.dest)
                .SelectMany(range => range.RangesNotInRanges(overlap.Select(o => o.source)))
                .Select(range => (range, range));
            map.Add(oneToOne.Concat(overlap).ToArray());
            i += rows.Length;
        }

        var result = map.Last().Select(r => r.dest.Begin).Min();
