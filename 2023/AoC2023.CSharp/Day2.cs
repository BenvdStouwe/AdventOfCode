...
public class Day2
{
    ...
    public void Part1()
    {
        var games = testInput.Parse();
        var result = games.Select(g =>
        {
            string[] gameParts = g.Split(':');
            var game = int.Parse(gameParts[0].Split(' ')[1]);
            var validCubes = gameParts[1].Split(new[] { ',', ';' }, StringSplitOptions.TrimEntries)
                .Select(c => c.Split(' '))
                .Select(c => ValidCube(c[1], int.Parse(c[0])))
                .All(v => v);
            return validCubes ? game : 0;
        })
        .Sum();
        ...
        var result = games.Select(g =>
            {
                string[] gameParts = g.Split(':');
                var game = int.Parse(gameParts[0].Split(' ')[1]);
                return gameParts[1].Split(';', StringSplitOptions.TrimEntries)
                    .SelectMany(g => g.Split(',', StringSplitOptions.TrimEntries))
                    .Select(c => c.Split(' ', StringSplitOptions.TrimEntries))
                    .Select(c => new {Color = c[1], Amount = int.Parse(c[0])})
                    .GroupBy(c => c.Color, c => c.Amount)
                    .Aggregate(1, (acc, cur) => acc * cur.Max());
            })
            .Sum();
        Assert.Equal(2286, result);
    }

    private bool ValidCube(string color, int amount)
    {
        return color switch
        {
            "blue" => amount <= 14,
