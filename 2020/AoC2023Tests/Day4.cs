...
public class Day4
{
    ...
    public void Part1()
    {
        var passports = testInput.Parse();
        var result = CountValidPassports(passports, false);
    ...
    public void Part2()
    {
        var map = part2TestInput.Parse();
        var result = CountValidPassports(map, true);
        Assert.Equal(4, result);
    }

    private static int CountValidPassports(string[] passports, bool validateValues) =>
        passports.Aggregate(new List<HashSet<KeyValuePair<string, string>>> { new() }, (acc, curr) =>
            {
                ...
                var set = acc.Last();
                foreach (var part in curr.Split(' ', StringSplitOptions.TrimEntries | StringSplitOptions.RemoveEmptyEntries))
                {
                    var pair = part.Split(':');
                    if (pair[0] == "cid" || validateValues && !ValidatePassportValue(pair[0], pair[1]))
                    {
                        continue;
                    }
                    set.Add(new(pair[0], pair[1]));
                }
                return acc;
            })
    ...
    private static bool ValidatePassportValue(string key, string value) =>
        key switch
        {
            "byr" => int.TryParse(value, out var year) && year >= 1920 && year <= 2002,
            "iyr" => int.TryParse(value, out var year) && year >= 2010 && year <= 2020,
            "eyr" => int.TryParse(value, out var year) && year >= 2020 && year <= 2030,
            "hgt" => 
                (value[^2..] == "in" && int.TryParse(value[..^2], out var heightInches) && heightInches >= 59 && heightInches <= 76)
                || (value[^2..] == "cm" && int.TryParse(value[..^2], out var heightCm) && heightCm >= 150 && heightCm <= 193),
            "hcl" => value[0] == '#' && long.TryParse(value[1..], NumberStyles.HexNumber, null, out _),
...
""";
    ...
    private const string part2TestInput = """
