...
public class Day2
{
    ...
    public void Part1()
    {
        var result = testInput.Parse()
            .Select(i => i.Split(' '))
            .Select(i => new {range = i[0].Split('-'), character = i[1][0], pass = i[2]})
            .Select(i => new Password(short.Parse(i.range[0]), short.Parse(i.range[1]), i.character, i.pass))
            .Select(p => (p.FirstIndex, p.SecondIndex, p.Value.Where(c => p.Character == c).Count()))
            .Count(p => p.Item3 >= p.FirstIndex && p.Item3 <= p.SecondIndex);
    ...
    public void Part2()
    {
        var result = testInput.Parse()
            ...
            .Select(i => new {range = i[0].Split('-'), character = i[1][0], pass = i[2]})
            .Select(i => new Password(short.Parse(i.range[0]), short.Parse(i.range[1]), i.character, i.pass))
            .Count(p => p.Value[p.FirstIndex - 1] == p.Character ^ p.Value[p.SecondIndex - 1] == p.Character);

        Assert.Equal(1, result);
    }

    private record struct Password(short FirstIndex, short SecondIndex, char Character, string Value);
